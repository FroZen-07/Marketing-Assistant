...
def getLLMResponse(query,age_option,tasktype_option, numberOfWords):
    ...
    if age_option=="Gen-Z": #Silly and Sweet Kid 
    ...
    prefix = """You are a {template_ageoption}: 
    Here are some examples: 
    """

    suffix = """
    Question: {template_tasktype_option} {template_userInput} in about {template_numberOfWords} word
    Response: """
    ...
    response=llm.invoke(new_prompt_template.format(template_userInput=query,template_ageoption=age_option,template_tasktype_option=tasktype_option, template_numberOfWords=numberOfWords))
    print(response)
...
st.set_page_config(page_title="Bitan's Marketing Tool",
                    page_icon='⛄',
                    layout='centered',
                    initial_sidebar_state='collapsed')
st.header("Welcome to Bitan's Marketing Assistant: ")

form_input = st.text_area('Enter Topic', height=200)

tasktype_option = st.selectbox(
    'Please select the action to be performed?',
    ('Write a Sales Copy', 'Create a Marketing Post for Social Media', 'Write a Product Description'),key=1)

age_option= st.selectbox(
    'For which age group?',
    ('Gen-Z', 'Adult', 'Senior Citizen'),key=2)
